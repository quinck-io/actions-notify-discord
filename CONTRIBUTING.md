...
- `pnpm install` rebuilds `dist/main.js` automatically (via the `prepare` script)
- `pnpm build` rebuilds it on demand
- `pnpm lint` runs oxlint (type-aware), `pnpm lint:fix` autofixes
- `pnpm fmt` formats with oxfmt, `pnpm fmt:check` verifies formatting
...
2.  rebuild if `src/` changed (`pnpm build`) and commit `src/` + `dist/` together
3.  merge to `main`
